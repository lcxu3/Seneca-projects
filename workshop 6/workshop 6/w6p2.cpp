...
void loadData(const char* filename, seneca::College& theCollege)
{
	...
	while (file )
	{
		...
		try {
			...
			std::cerr << "Error while extracting data: " << ex.what() << std::endl;
		}
	}
}
...
int main(int argc, char** argv)
{
	...
	std::list<const seneca::Person*> persons;
	{
		...
		auto students = [](const seneca::Person* p) {
			return dynamic_cast<const seneca::Student*>(p) != nullptr;
			};
	...
	persons.clear();
	{
		...
		auto professors= [](const seneca::Person* p) {
			return dynamic_cast<const seneca::Professor*>(p) != nullptr;
		};
